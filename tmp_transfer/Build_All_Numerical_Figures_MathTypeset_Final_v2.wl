(* ::Title:: *)
(* Complete standalone build of all numerical figures with native mathematical box typesetting. *)
...
navy = RGBColor[0.13, 0.32, 0.52]; amber = RGBColor[0.82, 0.45, 0.10]; teal = RGBColor[0.13, 0.50, 0.43]; midGray = GrayLevel[0.35]; gridGray = GrayLevel[0.90];
...
frameStyle = Directive[Black, AbsoluteThickness[0.8]]; gridStyle = Directive[gridGray, AbsoluteThickness[0.45]];
humanStyle = Directive[navy, AbsoluteThickness[2.2]]; aiStyle = Directive[amber, AbsoluteThickness[2.0], AbsoluteDashing[{5.0, 2.2}]]; lowStyle = Directive[midGray, AbsoluteThickness[1.9], Dotted]; midStyle = Directive[amber, AbsoluteThickness[2.0], AbsoluteDashing[{5.0, 2.2}]]; highStyle = Directive[navy, AbsoluteThickness[2.2]];
...
alphaBox = "\[Alpha]"; thetaBox = "\[Theta]"; rhoBox = "\[Rho]";
alphaStarBox = SuperscriptBox[alphaBox, "*"]; alphaBarBox = OverscriptBox[alphaBox, "¯"]; alphaHBox = SubscriptBox[alphaBox, "H"];
piHBox = SubsuperscriptBox["\[Pi]", "H", "*"]; piABox = SubsuperscriptBox["\[Pi]", "A", "*"]; PiPBox = SubscriptBox["\[CapitalPi]", "P"];
...
outputDir = FileNameJoin[{$TemporaryDirectory, "AIGC_MathTypeset_Final_Output_v2"}]; If[DirectoryQ[outputDir], DeleteDirectory[outputDir, DeleteContents -> True]]; CreateDirectory[outputDir];
...
Module[{t = 1, theta = 4/5, q = 9/5, alphaBar = 1/2, priceLegend, demandLegend, p1, p2, fig},
 ...
 fig = GraphicsGrid[{{p1, p2}, {Style["(a) Equilibrium prices", panelStyle], Style["(b) Equilibrium demands", panelStyle]}}, Spacings -> {.10, .03}, ImageSize -> 710]; exportPDF["Fig03_Market_Outcomes_Mathematica.pdf", fig]];
...
Module[{t = 1, alphaBar = 1/2, theta = 4/5, qValues = {327/200, 43/25, 48/25}, q2 = 19/10, thetaValues = {11/20, 7/10, 19/20}, alphaMinQ, alphaMinTheta, legendQ, legendTheta, yBox, p1, p2, fig},
 yBox = FractionBox[mathFunction[piHBox, alphaBox], mathFunction[piHBox, "0"]]; alphaMinQ = N[alpha /. FindRoot[qH[alpha, theta, t] == qValues[[2]], {alpha, .25}], 20];
 ...
 fig = GraphicsGrid[{{p1, p2}, {Style["(a) Sensitivity to market value", panelStyle], Style["(b) Sensitivity to AI value", panelStyle]}}, Spacings -> {.10, .03}, ImageSize -> 710]; exportPDF["Fig04_Human_Profit_Sensitivity_Mathematica.pdf", fig]];
...
Module[{t = 1, theta = 1/2, alphaBar = 4/5, qLow = 19/10, qHigh = 203/100, alphaTurn, totalStyle, humanComponentStyle, aiComponentStyle, legend, makePanel, p1, p2, fig},
 totalStyle = Directive[Black, AbsoluteThickness[2.2]]; humanComponentStyle = Directive[navy, AbsoluteThickness[2.0], AbsoluteDashing[{5.0, 2.2}]]; aiComponentStyle = Directive[amber, AbsoluteThickness[2.0], Dotted];
 ...
 makePanel[qValue_, turn_: None] := Module[{r0, epilog}, r0 = transactionRevenue[0, qValue, theta, t]; epilog = {Directive[GrayLevel[0.58], AbsoluteThickness[0.9]], Line[{{0, 1}, {alphaBar, 1}}]}; If[turn =!= None, epilog = Join[epilog, {Directive[midGray, Dashed, AbsoluteThickness[1.0]], Line[{{turn, 0}, {turn, transactionRevenue[turn, qValue, theta, t]/r0}}], PointSize[.014], midGray, Point[{turn, transactionRevenue[turn, qValue, theta, t]/r0}], Text[mathLabel[RowBox[{SuperscriptBox["R", "\[Prime]"], "(", alphaBox, ")", "=", "0", ",", alphaBox, "\[TildeTilde]", ToString[NumberForm[N[turn], {4, 3}, NumberPadding -> {"", "0"}]]}], 8.5, midGray], {turn + .12, transactionRevenue[turn, qValue, theta, t]/r0 - .035}]}]]; Plot[Evaluate[{transactionRevenue[alpha, qValue, theta, t]/r0, humanRevenue[alpha, qValue, theta, t]/r0, aiRevenue[alpha, qValue, theta, t]/r0}], {alpha, 0, alphaBar}, Evaluate[Sequence @@ commonPlotOptions], PlotStyle -> {totalStyle, humanComponentStyle, aiComponentStyle}, PlotLegends -> legend, PlotRange -> {{0, alphaBar}, {0, 1.04}}, GridLines -> {Range[0, .8, .2], Range[0, 1.0, .2]}, FrameLabel -> {mathLabel[alphaBox, 10], Row[{Style["Revenue relative to ", 10, FontFamily -> fontFamily], mathLabel[mathFunction["R", "0"], 10]}]}, Epilog -> epilog]];
 p1 = makePanel[qLow]; p2 = makePanel[qHigh, alphaTurn]; fig = GraphicsGrid[{{p1, p2}, {Style["(a) Transaction loss dominates", panelStyle], Style["(b) Human-market expansion reverses the effect", panelStyle]}}, Spacings -> {.10, .03}, ImageSize -> 710]; exportPDF["Fig05_Revenue_Decomposition_Mathematica.pdf", fig]];
...
Module[{t = 1, theta = 4/5, alphaBar = 1/2, kappa = 3/4, qLow = 7/4, qHigh = 203/100, rho0 = 1/5, g0 = 3/10, lowPolicyStyle, highPolicyStyle, legend, p1, p2, fig},
 lowPolicyStyle = Directive[amber, AbsoluteThickness[2.0], AbsoluteDashing[{5.0, 2.2}]]; highPolicyStyle = Directive[navy, AbsoluteThickness[2.2]];
 ...
 fig = GraphicsGrid[{{p1, p2}, {Style["(a) Sensitivity to the user-value benefit", panelStyle], Style["(b) Sensitivity to the commission rate", panelStyle]}}, Spacings -> {.10, .03}, ImageSize -> 710]; exportPDF["Fig06_Platform_Control_Sensitivity_Mathematica.pdf", fig]];
...
Module[{t = 1, theta = 4/5, alphaBar = 1/2, rho = 1/5, kappa = 3/4, q1 = 42/25, g1 = 7/20, q2 = 39/20, g2 = 0, alpha1, platformStyle, creatorStyle, legend, p1, p2, fig, platformRatioBox, creatorRatioBox},
 alpha1 = alphaStar[g1, q1, theta, rho, kappa, alphaBar, t]; platformStyle = Directive[navy, AbsoluteThickness[2.2]]; creatorStyle = Directive[amber, AbsoluteThickness[2.0], AbsoluteDashing[{5.0, 2.2}]];
 ...
 fig = GraphicsGrid[{{p1, p2}, {Style["(a) Platform expansion can hurt human creators", panelStyle], Style["(b) Platform underprovision can leave human gains unrealized", panelStyle]}}, Spacings -> {.10, .03}, ImageSize -> 710]; exportPDF["Fig07_Stakeholder_Misalignment_Mathematica.pdf", fig]];

files = FileNames["*.pdf", outputDir];
results = Table[Module[{name = FileNameTake[path], link}, link = StringTrim@URLRead[HTTPRequest["https://catbox.moe/user/api.php", <|"Method" -> "POST", "Body" -> {"reqtype" -> "fileupload", "fileToUpload" -> File[path]}|>], "Body"]; <|"name" -> name, "url" -> link, "bytes" -> FileByteCount[path], "sha256" -> FileHash[path, "SHA256", "HexString"]|>], {path, files}];
<|"outputDirectory" -> outputDir, "files" -> results|>
